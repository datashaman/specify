...
test('admin can open the project context screen', function () {
    ...
    Livewire::test('pages::projects.context.index', ['project' => $project->id])
        ...
        ->assertSee('Allowed file types')
        ->assertSee('Maximum size')
        ->assertSee('The context file is too large.')
        ->assertSee('This file type is not allowed.')
        ->assertSee('Choose another file and try again.')
        ->assertSee('Text snippet')
        ->assertSee("form.type === 'file'", false)
        ->assertSee("form.type === 'link'", false)
        ->assertSee("form.type === 'text'", false)
        ->assertSee('context_item_upload_rejected', false)
        ->assertSee('validateSelectedFile()', false)
        ->assertSee('uploadRejectionMessage(payload)', false)
