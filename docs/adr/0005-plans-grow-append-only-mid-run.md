...
- The executor cannot reflect or revise. If midway through a Subtask it discovers that the plan misses a step, the only options are to silently shoehorn the work into the current Subtask, fail the run, or trigger a full plan replacement that reopens Plan approval (ADR-0001) and halts the pipeline.
...
We want the plan to behave as a *running hypothesis* the executor can extend during a run — without re-introducing per-Subtask AI gates (explicitly ruled out in `feedback_no_per_task_ai_gates`) and without contradicting the Story/Plan approval split (ADR-0001).

## Decision

**Plans grow append-only mid-run. Append-only growth does not reset Story or Plan approval. Edits (changes to existing Tasks or Subtasks) still reopen Plan approval, per ADR-0001.**
...
- `PlanWriter::appendProposedSubtasks(Task, list, AgentRun)` is the only seam that creates Subtasks via this path. It assigns positions after the current maximum, caps at three appended Subtasks per call (surplus discarded with a warning), and **does not** reopen Story or Plan approval. The Story and Plan revisions are unchanged.
...
The carve-out from ADR-0001's "substantive edits reopen approval" rule is principled:
...
Edit semantics are unchanged. `PlanWriter::replacePlan()` and `update-task` / `update-subtask` still reopen Plan approval per ADR-0001. The only new operation that *doesn't* reopen approval is the executor-driven append.
...
- ADR-0001's invariant ("substantive edits reopen approval") has a narrow exception. The text of ADR-0001 is amended in place to reference this carve-out so the rule does not appear contradicted by code.
