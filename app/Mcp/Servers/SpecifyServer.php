...
- A TASK is the engineering contract for delivering one acceptance criterion. SUBTASKS are the engineering breakdown of a task; the executor runs one subtask at a time. Use set-tasks to write the full task list for a story (one task per AC, each with 1+ subtasks). Use update-task / update-subtask for surgical edits. NEVER put schemas, class names, file paths, or migration steps in a feature or story description — that belongs in tasks/subtasks.

Editing the task list of an Approved story automatically resets it to PendingApproval — story approval is the only gate before execution.
